...
let NgbdtypeheadBasicComponent = class NgbdtypeheadBasicComponent {
    constructor(_service) {
        ...
        this.search1 = (text$) => text$.pipe(debounceTime(200), distinctUntilChanged(), map(term => (term.length < 2 ? [] : states.filter(v => v.toLowerCase().indexOf(term.toLowerCase()) > -1).slice(0, 10)))
        ...
        this.search3 = (text$) => text$.pipe(debounceTime(200), distinctUntilChanged(), map(term => (term === '' ? [] : states.filter(v => v.toLowerCase().indexOf(term.toLowerCase()) > -1).slice(0, 10)))
        // tslint:disable-next-line:semicolon
        );
        ...
        this.search5 = (text$) => text$.pipe(debounceTime(200), map(term => (term === '' ? [] : statesWithFlags.filter(v => v.name.toLowerCase().indexOf(term.toLowerCase()) > -1).slice(0, 10)))
        // tslint:disable-next-line:semicolon
        );
...
export { NgbdtypeheadBasicComponent };
